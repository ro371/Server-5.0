...
public class BuyguiSlotMessage {
	private final int slotID, x, y, z, changeType, meta;

	public BuyguiSlotMessage(int slotID, int x, int y, int z, int changeType, int meta) {
		this.slotID = slotID;
		this.x = x;
		this.y = y;
		this.z = z;
	...
	public BuyguiSlotMessage(FriendlyByteBuf buffer) {
	...
	public static void buffer(BuyguiSlotMessage message, FriendlyByteBuf buffer) {
	...
	public static void handler(BuyguiSlotMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
	...
	public static void handleSlotAction(Player entity, int slotID, int changeType, int meta, int x, int y, int z) {
		...
		if (slotID == 5 && changeType == 1) {
			...
			TradedoneProcedure.execute(world, entity);
		}
	}
